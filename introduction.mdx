---
description: "Aippy — your AI-powered teammate for bringing ideas to working projects."
title: "Welcome"
"icon": "house"
---

## 

import { discordInviteLink, DiscordLink } from '/snippets/discord.mdx'

Aippy is your AI-powered teammate for full-stack creation.
Just describe what you want to build — Aippy turns your ideas into working apps, websites, and tools. No code required.
Aippy handles the tech — you stay in the flow.

## Product Capabilities

![Aippy application overview](/images/aippy_home.png)

Aippy gives you everything you need to turn ideas into working projects — right from your browser. Whether it’s a landing page or a full-stack app, Aippy helps you create, remix, and share — all from a single browser tab.

| Capability                    | Description                                                                 | Maturity Level    |
|------------------------------|-----------------------------------------------------------------------------|-------------------|
| **Prompt-to-App Generation** | Generate functional frontend projects from natural language input.          | 🟢 Mature          |
| **Live Preview & History Tracking** | Real-time rendering with one-click version rollback.                | 🟢 Mature          |
| **Instant Deployment**       | Push projects live with minimal setup across environments.                  | 🟢 Mature          |
| **Full-Stack Support**       | Backend logic, database integration.                                        | 🛠️ Building         |
| **Instant UI Editing**       | Edit text and styles directly on the page.                                  | 🛠️ Building         |
| **More Deployment Options**  | Easily publish projects with custom domains, enable payments, and connect to external services. | 🛠️ Building         |


## Quick Start

To create your application on Aippy, choose the guide that suits your needs:

<CardGroup cols="2">

<Card
  title="Describe Your Idea"
  icon="message-square"
  href="/user-guides/prompting"
>
  Start with a simple prompt in your own language. Aippy will instantly turn it into a working web app.
</Card>

<Card
  title="Edit & Refine"
  icon="wand"
  href="/guides/instant-editing"
>
  Click directly on the canvas to make changes, or chat with AI to update layout and logic.
</Card>

<Card
  title="Live Preview"
  icon="eye"
  href="/features/preview"
>
  See real-time changes as you build — no refresh or rebuild needed.
</Card>

<Card
  title="Version History"
  icon="history"
  href="/features/versioning"
>
  Every change is saved. Go back to any previous version with one click.
</Card>

<Card
  title="Publish Instantly"
  icon="rocket"
  href="/features/deploy"
>
  Launch your project on a unique URL in seconds — no config required.
</Card>

<Card
  title="Custom Domain"
  icon="globe"
  href="/tips-tricks/custom-domain"
>
  Link your own domain to make your Aippy app truly yours.
</Card>

<Card
  title="Extend with Backend"
  icon="server"
  href="/guides/full-stack"
>
  Add backend logic, APIs, and persistent data with one click.
</Card>

<Card
  title="Payments & Auth"
  icon="lock-keyhole"
  href="/guides/integrations"
>
  Integrate Stripe, Supabase, and other tools for monetization and user login.
</Card>

<Card
  title="Explore Templates"
  icon="layers"
  href="/templates"
>
  Remix from curated showcases to kickstart your next idea.
</Card>

</CardGroup>

## Support

<CardGroup cols="2">
  <Card title="Community Support" icon="discord" href="https://discord.gg/keESZDHq ">
    Get help from our Community and Community Champions.
  </Card>
  <Card title="Issues" icon="bug" href="https://feedback.aippy.ai/">
    Found a bug or an issue? Let us know.
  </Card>
  <Card title="Priority Support" icon="headset" href="https://aippy.ai/support">
    Direct support channel for paying users.
  </Card>
</CardGroup>

## Say hello to Aippy

<Tip>
  **AIPPY**  
  *Allowing Innovative People to Program Instantly with Your ideas*
</Tip>

**Let your idea flow.**  
**Aippy** is your AI-powered teammate for bringing ideas to working projects — fast, flexible, and all in one browser tab.

Whether you're sketching out a landing page, launching a product MVP, or building something just for fun, Aippy helps you turn thoughts into live applications — no setup, no blockers.

Use natural language to generate complete UIs.  
Click to edit with instant visual tools.  
Or chat with Aippy to refine layouts, logic, and content — like you're working with a creative partner who codes.

### What Aippy helps you do:
- Generate full apps from plain-language prompts  
- Instantly edit text and styles right on the page  
- Chat with AI to adjust layout, structure, and logic  
- Catch and fix issues automatically  
- Deploy with one click — and connect your own domain  
- Store data with built-in Supabase integration  
- Share, remix, and grow your ideas in public or private

There’s no one way to build — and with Aippy, you don’t need to choose.  
**Just start creating, and let your idea flow.**


## Stay Connected

<CardGroup cols="2">
  <Card title="Feature requests" icon="bell-concierge" iconType="regular" href="https://feedback.aippy.ai/">
    Have an idea? Share it and let the community vote.
  </Card>
  <Card title="Partner program" icon="handshake" href="https://aippy.dev/partners">
    Join as an expert, or get help from our network of experts.
  </Card>
  <Card title="Follow on X" icon="x" href="https://x.com/aippyai">
    Follow us on [X](https://x.com/aippyai) for the latest updates and announcements.
  </Card>
  <Card title="Product Hunt" icon="product-hunt" href="https://www.producthunt.com/p/aippy">
    Upvote on our product launches.
  </Card>
</CardGroup>

